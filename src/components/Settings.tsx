...
export function Settings({ config, onConfig, onClear, onInstall, installed }: { config: TimerConfig; onConfig: (config: TimerConfig) => void; onClear: () => void; onInstall: () => void; installed: boolean }) {
  ...
  return <><h1 className="sr-only">Settings</h1>
    ...
    <section className="card settings-card"><ShieldCheck size={24}/><h2>Your practice stays here.</h2><p>No accounts. No uploads. Microphone audio is processed as it arrives and is never recorded. Waveforms use temporary level measurements.</p><p className="helper">Only your profiles, settings, and latest 100 strings are saved. Clearing browser data or uninstalling may remove them.</p></section>
    <section className="card settings-card"><Trash2 size={24}/><h2>A fresh start</h2><p>Remove all settings, calibration profiles, and saved strings from this device.</p><button className="button secondary danger" onClick={() => { if (window.confirm('Delete all Shot Timer settings, calibration profiles, and saved strings? This cannot be undone.')) onClear(); }}>Clear all stored data</button></section></div>
    <p className="settings-footnote">Shot Timer / v1.0 · Built for semi-auto airsoft practice. Keep the app visible while timing.</p>
  </>;
}
