...
export function Settings({ config, debugMode, onDebugMode, onConfig, onClear, onInstall, installed }: { config: TimerConfig; debugMode: boolean; onDebugMode: (enabled: boolean) => void; onConfig: (config: TimerConfig) => void; onClear: () => void; onInstall: () => void; installed: boolean }) {
  ...
  return <><h1 className="sr-only">Settings</h1>
    ...
    <section className="card settings-card"><h2>Debug mode</h2><button type="button" className="button secondary debug-toggle" aria-label="Debug mode" aria-pressed={debugMode} onClick={() => onDebugMode(!debugMode)}>{debugMode ? 'ON' : 'OFF'}</button><p className="debug-description">Show live and review waveforms for new stages.</p><p className="helper">The latest three debug waveforms stay in memory until you reload or turn Debug mode off. No audio is recorded.</p></section>
